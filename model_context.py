...
def get_stablelm_context():
    ...
    service_context = ServiceContext.from_defaults(chunk_size=1024, llm=hf_predictor, embed_model="local")
...
def get_falcon_context():
    ...
    hf_predictor = HuggingFaceLLM(
        ...
        model_kwargs={"trust_remote_code": True}#, "load_in_8bit": True} #"torch_dtype": torch.float16
    )
    service_context = ServiceContext.from_defaults(chunk_size=1024, llm=hf_predictor, embed_model='local', query_wrapper_prompt=query_wrapper_prompt)
    return service_context
